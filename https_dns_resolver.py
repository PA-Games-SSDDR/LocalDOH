...
from flask import Flask, request, redirect, Response, render_template, abort, jsonify
import requests
import os
import json
...
def decode_message_jsonify(message):    
    res = []
    dict_res = {}
    ...
    res.append("QTYPE: " + QTYPE + " (\"" + get_type(int(QTYPE, 16)) + "\")")
    res.append("QCLASS: " + QCLASS)
    dict_res["Question"] = [{"name": QNAME, "type": int(QTYPE)}]
    dict_res["Answer"] = []
    ...
    if NUM_ANSWERS > 0:
        ...
        for ANSWER_COUNT in range(NUM_ANSWERS):
            ...
            try: ATYPE
                ...
                res.append("RDDATA: " + RDDATA)
                res.append("RDDATA decoded (result): " + RDDATA_decoded + "\n")
                dict_res["Answer"] += [{"name": QNAME, "type": int(ATYPE), "TTL": int(TTL), "data": RDDATA_decoded}]
...
# response = send_udp_message(message, "192.168.1.103", 53)
# print("\nResponse:\n" + response)
...
def get_hostname_str(k=""):
    return str(urlparse(k).hostname)
...
def index():
    ...
    # # second argument is external DNS server, third argument is port
    response = send_udp_message(message, "192.168.1.103", 53)
    # # second argument is interna DNS server, third argument is port
    # response = send_udp_message(message, "127.0.0.1", 53)
    print("\nResponse:\n" + response)
    print("\nResponse (decoded):" + decode_message(response))
    # print("\nJsonify: ", jsonify(decode_message_jsonify(response)))
    return Response(json.dumps(decode_message_jsonify(response)), mimetype='application/dns+json')
